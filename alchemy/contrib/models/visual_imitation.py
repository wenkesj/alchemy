...
from alchemy.layers import autoencoder
from alchemy.utils import array_utils
from alchemy.contrib.models import zero_shot


class VisualImitationModel(zero_shot.ZeroShotImitationModel):
  ...
  def build_state_embedding_op(self,
    ...
    (outputs, states, initial_state_phs, zero_states,
     encoder, shapes, output_shape, hidden_sizes) = autoencoder.conv2d_rnn_encoder(
        self.state_ph,
  ...
  def build_goal_state_embedding_op(self,
                                    ...
                                    rnn_hidden_sizes=[1],
                                    rnn_cell_fn=tf.contrib.rnn.BasicLSTMCell):
    (outputs, states, initial_state_phs, zero_states, _, _, _, _) = autoencoder.conv2d_rnn_encoder(
        self.goal_state_ph,
        self.state_space.shape,
        filters,
        kernel_sizes,
  ...
  def build_action_value_op(self,
    ...
    with tf.variable_scope('action_value'):
      ...
      x = tf.layers.dense(x, array_utils.product(self.action_space.shape))
