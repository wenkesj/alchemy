...
from alchemy.utils import mask_sequence, nd_expand_dims, all_equal, ssd, safe_tf_dtype
...
class ZeroShotImitationModel(ABC):
  ...
  def build_next_state_loss_op(self, **kwargs):
    return .5 * ssd(self.next_state_op, self.next_state_ph)
  ...
  def zero_state_op(self, batch_size, dtype=tf.float32):
    states = [fn(batch_size, safe_tf_dtype(dtype)) for fn in self.zero_state_fns]
    return states
  ...
  def next_state_feed_dict(self, state, action_value, initial_state=[], extra_feed_dict={}):
    assert all_equal(state.shape, self.state_space.shape)
    assert all_equal(action_value.shape, self.action_space.shape)

    internal_state = {k: v for k, v in zip(self.internal_state_phs, initial_state)}
    return {
      self.state_ph: nd_expand_dims(state, 2),
      self.action_value_ph: nd_expand_dims(action_value, 2),
      **internal_state,
      **extra_feed_dict,
    }
  ...
  def action_value_feed_dict(self, state, goal_state, initial_state=[], extra_feed_dict={}):
    assert all_equal(state.shape, self.state_space.shape)
    assert all_equal(goal_state.shape, self.state_space.shape)

    internal_state = {k: v for k, v in zip(self.internal_state_phs, initial_state)}
    return {
      self.state_ph: nd_expand_dims(state, 2),
      self.goal_state_ph: nd_expand_dims(goal_state, 2),
      **internal_state,
      **extra_feed_dict,
    }
